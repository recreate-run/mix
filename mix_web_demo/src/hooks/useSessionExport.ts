	...
	return useMutation<unknown, Error, ExportSessionOptions>({
		mutationFn: async ({ sessionId }) => {
			...
			if (PlatformFeatures.hasNativeDialogs()) {
				// Desktop-only feature: Native save dialog using Tauri
				// This is disabled in the web version as @tauri-apps packages are not available
				throw new Error("Native save dialog is only available in desktop version. Using browser download instead.");

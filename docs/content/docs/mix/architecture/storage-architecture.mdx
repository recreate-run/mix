...
Storage provider is configured via environment variables in `.env`:
...
STORAGE_ACCESS_KEY=your_service_role_key
```

All file operations (upload, download, delete, list) automatically use the configured provider, making storage backend transparent to application logic.
...
│   ├── image.jpg
│   └── data.csv
├── thumbnails/        # Generated thumbnails (automatic)
│   ├── {hash}_box200_t1.0.jpg
│   └── {hash}_w300.jpg
...
When using Supabase, files are stored with the following key structure:

```
uploads/{filename}              # User-uploaded files
thumbnails/{hash}_{spec}.jpg    # Generated thumbnails
```

Thumbnails are automatically filtered from file listings and are not visible to users in the @ command.

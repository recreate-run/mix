...
import { convertBackendMessagesToUI, type BackendMessage, type UIMessage } from '@/lib/messageUtils';

const loadSessionMessages = async (sessionId: string): Promise<BackendMessage[]> => {
  const result = await rpcCall<BackendMessage[]>('messages.list', { sessionId });
  return result || [];
};

const loadAndConvertMessages = async (sessionId: string): Promise<UIMessage[]> => {
  try {
